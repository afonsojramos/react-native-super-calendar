...
| Prop                    | Type                                     | Notes                                                                             |
| ----------------------- | ---------------------------------------- | --------------------------------------------------------------------------------- |
| `onDragEvent`           | `(event, start, end) => void \| boolean` | Enable move/resize (month: move by whole days); return `false` to reject.         |
| `onDragStart`           | `(event) => void`                        | Fires on grab (e.g. haptics).                                                     |
| `onCreateEvent`         | `(start, end) => void`                   | Drag empty space to create (month/year: an all-day span).                         |
| `onSelectDrag`          | `(start, end) => void`                   | Month/year: drag to sweep a date range; fires live.                               |
