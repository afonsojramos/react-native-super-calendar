...
across month boundaries — something the horizontally-paged month view can't do.
An events-free picker needs nothing but `date` and the selection wiring:
...
Pass `events` (plus an optional `renderEvent`) to show events in the grids too —
it reuses `MonthView`, so events, selection, and disabled days behave exactly as
in the month view.
