...
<Tip>
  For a picker-only app, import from the `react-native-super-calendar/picker` entry point. It
  excludes the timetable views and Reanimated, so the bundle stays small and you can skip the
