...
By default each resource header shows its `title` (falling back to the `id`). To
render it yourself, an avatar, a subtitle, a status dot, a capacity count, pass
`renderResourceHeader`. It receives the resource and its index and returns the
cell's content; the board keeps the header cell's sizing so the columns stay
aligned with the lanes below. It applies to both orientations: the top header
cells when `orientation="vertical"`, the left row labels when horizontal.
...
<ResourceTimeline
  ...
  renderResourceHeader={(resource) => (
    <ResourceLabel name={resource.title} avatar={resource.avatarUrl} />
  )}
/>
```
...
`renderBusinessHours`, `renderResourceHeader`), plus
