...
state in response. Most of it lives on the week/day grid; the month and year
grids get their own day-granular versions, covered at the end.
...
## On the month and year grids

The month and year grids work in whole days rather than minutes, so the same three
handlers do day-granular versions of the gestures above. On a device you **hold and
drag**; on the web you **press and drag**.

### Sweep out a span

`onCreateEvent` sketches an **all-day** span: start on an empty day and drag across
others. `start` is midnight of the first day and `end` is midnight after the last
(exclusive), which is the shape an all-day event wants. A plain tap still fires
`onPressDay`, and each day in the sweep carries `data-creating` for styling (see
...
`[start, end]` days, so a selection highlight can follow the drag. Set either
handler (or both) to enable the gesture — see
[Date selection](/guides/selection) for wiring it to `useDateRange`.

On the year grid the sweep runs across a mini month's days on a device, and across
mini months on the web.

### Move an event to another day

In `month` mode, `onDragEvent` picks an event chip up and drops it on another day.
The event shifts by whole days, keeping its **time of day and its duration**, so a
9–10am standup dropped three days later is still 9–10am. Multi-day bars move as a
whole, from wherever you grabbed them.
...
`draggable: false`, `startEditable: false`, and `disabled: true` lock an event here
exactly as they do on the time grid, and `onDragStart` fires on grab for haptics.
The year view summarises events as dots rather than chips, so it has nothing to
pick up: there is no drag-to-move there.
