...
Also pass `onChangeDate` alongside `onDragEvent` to let a drag cross the visible
range into an off-screen period. Drag an event to the left or right edge of the
day columns and hold briefly; the view pages to the previous/next period and
keeps paging while you hold there.

- **Web:** the drag stays live across the page change; drop on any day in the
  newly revealed range.
- **React Native:** the event lifts into a floating ghost, the edge dwell pages
  the view, and you tap the target day and time on the new page to place it (the
  lift survives the page change because it sits above the pager).

It's automatic whenever both `onChangeDate` and `onDragEvent` are set, and only
fires on a deliberate dwell at the very edge, so it won't trip a normal in-view
drag.
